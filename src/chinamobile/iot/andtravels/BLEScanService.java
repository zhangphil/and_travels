...
public class BLEScanService extends Service implements Runnable{
	...
	private String getRecentBeacon(ArrayList<Beacon> list){
		...
		for(int i=0;i<list.size();i++){
			...
			Log.e(TAG, "找到的beacon name: " +beacon.getName() + "distance：" + beacon.getDistance() + "Size:" + list.size() + " minDistance " + minDistance) ;
			 
		}
		
		Log.e(TAG, "找到index:" + minIndex);
		if(minIndex >= 0){
			//strBeaconID = ((Beacon)list.get(index).get("beacon")).getProximityUUID();
    	                ...
    	                try{
    	                	 if(response.getString("code").equals("1")){
    	                		 ...
    	                		 for(int i= 0; i < jsonArryData.length();i++){
    	                			 final String soundUrl=(String)((JSONObject) jsonArryData.getJSONObject(i)).getString("url");
    	                			 playMedia(soundUrl);
    	                			 stopScanBle();
    	                		 }
    	    	                }
    ...
    public void playMedia(String url){
    	//url ="http://play.baidu.com/?__m=mboxCtrl.playSong&__a=130238461&__o=song/130238461||playBtn&fr=-1||www.sowang.com#";
